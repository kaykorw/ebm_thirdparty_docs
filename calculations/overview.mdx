...
description: "How Kayko EBM amounts are computed from line items to invoice totals. Header and line values must match or RRA returns error 910."
keywords:
  - EBM calculations
  - invoice totals
  - VAT math
...
<CardGroup cols={2}>
  <Card title="VAT Formulas" icon="percent" href="/calculations/vat-formulas">
    Tax categories A–D plus fuel F, the 18/118 extraction formula, and multi-category invoices.
  </Card>
  <Card title="Tourism & Fuel" icon="gas-pump" href="/calculations/tourism-and-fuel">
    Tourism tax (`use_tourism_tax` / `ttAmt`) versus VAT on fuel (`tax_type` F, RRP, sale purpose).
  </Card>
...
| Live rates F/TT | Header always includes `taxRtF` (18) and `taxRtTt` (0 or 3). Fuel amounts use bucket F. Tourism amounts use `ttAmt` / `totalTTAmt`, not line `tax_type` TT. |
