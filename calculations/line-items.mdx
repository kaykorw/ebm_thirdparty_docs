...
| `taxTyCd` | Formula |
|-----------|---------|
| `"B"` or `"F"` | `ROUND(taxblAmt × 18 / 118, 2)` |
| `"A"`, `"C"`, `"D"` | `0` |
...
- [ ] `totAmt = taxblAmt`
- [ ] `tax_type` matches the item's registered tax type from `POST /items` (fuel is `F`; tourism stays A–D plus `use_tourism_tax`)
- [ ] Fuel lines include `rrp` / `sale_purpose` and no normal discount; see [Tourism Tax and VAT on Fuel](/calculations/tourism-and-fuel)
