...
description: "How discount_rate, discount_amount, and total_discount_amount affect taxable amounts on Kayko EBM sales invoices."
keywords:
  - EBM discounts
  - discount_rate
  - discount_amount
...
Discounts reduce the taxable base before VAT is extracted. On Kayko requests send `discount_rate`, `discount_amount`, and `total_discount_amount`. Kayko maps them to VSDC `dcRt`, `dcAmt`, and `totDcAmt`.

| Kayko field | VSDC | Type | Description |
|-------------|------|------|-------------|
| `discount_rate` | `dcRt` | % | Percentage discount (0–100) |
| `discount_amount` | `dcAmt` | RWF | Absolute discount value |
| `total_discount_amount` | `totDcAmt` | RWF | Total discount applied to the line |

When no discount applies, omit them or send `0`. Kayko fills missing pairs (rate from amount, or amount from rate).
...
Fuel lines cannot use these normal discounts. Use `regulated_discount` (0–120) instead. See [Tourism Tax and VAT on Fuel](/calculations/tourism-and-fuel).
