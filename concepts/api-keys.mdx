...
description: Enable EBM in the Kayko business dashboard, generate an API key, and authenticate every VSDC request.
icon: key
---

Kayko protects VSDC access with a **business API key**. Device initialization still issues RRA signing keys (`intrlKey`, `signKey`, `cmcKey`)- those are for receipt crypto. The API key is what your integration sends on **every** request to prove it belongs to your business.
...
<Steps>
  ...
  <Step title="Enable EBM">
    Turn on **EBM**. This provisions your business for VSDC (`enable_ebm`) and assigns your business `ebm_url`. You cannot create an EBM API key until this step succeeds.
  </Step>
...
```bash cURL
curl -X POST "{{baseUrl}}/initializer/selectInitInfo" \
...
const response = await fetch(`${BASE_URL}/initializer/selectInitInfo`, {
  method: 'POST',
  headers: {
...
```php PHP
<?php
$ch = curl_init($baseUrl . '/initializer/selectInitInfo');
...
|------------|-----------|----------|
| Business API key (`sk_…`) | Kayko dashboard | Authenticate your app to Kayko VSDC / EBM on each HTTP request |
| `intrlKey` / `signKey` / `cmcKey` | RRA via `selectInitInfo` | Receipt signing and VSDC device crypto (managed by VSDC) |
...
<CardGroup cols={2}>
  <Card title="Quickstart" icon="bolt" href="/quickstart">
    Enable EBM, generate a key, initialize the device, and make your first call.
  </Card>
