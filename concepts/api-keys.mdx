...
A request fails with `API_KEY_SCOPE_DENIED` if the key is missing the scope required by that endpoint. Constants currently accept any valid EBM API key (no scope check); other domains enforce the scopes above.
