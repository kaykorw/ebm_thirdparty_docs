...
description: Authenticate Kayko EBM requests with a business API key from the Kayko dashboard. Covers Enable EBM, scopes, Bearer tokens, and x-api-key.
keywords:
  - Kayko API key
  - EBM authentication
  - Bearer sk_live
...
<Steps>
  ...
  <Step title="Generate an API key">
    ...
    <Warning>
      The full secret (`sk_live_…` or `sk_test_…`) is shown **once**. Store it in a secrets manager immediately. Kayko only keeps a hash, so you cannot retrieve the plaintext later. Rotate the key if it is lost.
    </Warning>
  </Step>
</Steps>
...
```php PHP
<?php
$ch = curl_init($baseUrl . '/customers?customer_tin=100004527');
...
Initialization routes (`/initialize/*`) prefer the internal `X-API-Token` (a business API key also works). Core360 uses the internal token when you Enable EBM. See [Initialization](/api/initialization).
...
| `branch` | Branch list, users, insurance, and related branch pushes |
| `items` | Item registration, catalog list, get/update/deactivate, and item classes |
| `stocks` | Stock movements, stock master, automatic chain after sales/purchases/imports |
| `sales` | Sales, refunds, resend, and invoice lookup |
| `purchase` | Purchase list and confirmation |
| `notices` | Notices pulls |
| `import` | Customs import list and confirm |
| `codes` | Standard code sync and tourism tax settings (`/codes`, `/codes/cached`, `/codes/tourism`) |

A request fails with `API_KEY_SCOPE_DENIED` if the key is missing the scope required by that endpoint. Constants and live `/codes` routes currently accept any valid EBM API key (no scope check); other domains enforce the scopes above.
...
<CardGroup cols={2}>
  ...
  <Card title="Integration Checklist" icon="list-ol" href="/workflows/getting-started">
