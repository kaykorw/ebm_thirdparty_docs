...
Many VSDC endpoints are **incremental sync** APIs- they only return records that changed since your last request. Understanding this pattern is critical for a reliable integration.
...
sequenceDiagram
    ...
    App->>VSDC: selectCodes (lastReqDt: "20000101000000")
    VSDC-->>App: All codes + resultDt: "20260303120000"
    Note over App: Store "20260303120000"

    App->>VSDC: selectCodes (lastReqDt: "20260303120000")
    VSDC-->>App: Only changed codes + resultDt: "20260304100000"
...
async function syncCodes(tin, bhfId, lastReqDt) {
  const response = await vsdcPost('/code/selectCodes', { tin, bhfId, lastReqDt });
  ...
  throw new VSDCError(response.resultCd, response.resultMsg);
}
```
...
VSDC can issue receipts offline, but with a hard limit:

> If there is no internet connection for **24 hours** after VSDC issues a receipt signature, VSDC **stops issuing receipt numbers**.
