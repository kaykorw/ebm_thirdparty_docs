...
| `tourism_tax` | No | When `true`, Kayko uses catalog tourism header rates. When false/omitted, non-tourism null strategy applies unless sale-level overrides exist. |
| `tax_rate_f` / `tax_rate_tt` | No | Optional numeric overrides (0–100) for this sale only. |
...
The response is `{ message, data }`. On success, `data` includes `invoice_no`, `original_invoice_no`, `receipt_type`, `status`, `replayed`, and a `print` object with `receipt_no`, `total_receipt_no`, `internal_data`, `receipt_signature`, `receipt_published_at`, `sdc_id`, and `mrc_no`.
...
| `910` | Parameter validation (totals, `orgInvcNo`, tourism rates, types) | Fix payload; for tourism rates use sale-level `tax_rate_f` / `tax_rate_tt` |
