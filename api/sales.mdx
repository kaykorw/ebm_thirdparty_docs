...
The response is `{ message, data }`. On a live `POST /sales` or resend, `ebm` is also included (the VSDC `saveSales` envelope). `data` includes invoice identity, `customer`, `items` (from the stored request), `totals`, `print`, and related sale fields. `GET /sales/:invoice_no` does not call the device, so it has no `ebm` key.
...
Returns the **locally stored** invoice (not a live VSDC lookup). VSDC `saveSales` only returns print fields; Kayko keeps the request body (`itemList`, customer, totals) and maps it back here.

Includes `items`, `customer`, `purchase_code`, `payment_method`, `dates`, `totals`, and `print` from the local store. This route does not call VSDC, so there is no `ebm` object.

Requires the same tenant headers and `sales` scope.
