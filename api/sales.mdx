...
| `tourism_tax` | No | When `true`, Kayko uses catalog tourism header rates. When false/omitted, non-tourism null strategy applies unless overrides exist. Configure defaults via [`PUT /codes/tourism`](/api/codes). |
| `tax_rate_f` / `tax_rate_tt` | No | Optional numeric overrides (0–100) for this sale only. Prefer configuring defaults via [`PUT /codes/tourism`](/api/codes). |
...
Do **not** send sale line `tax_type: "F"`. Use **`TT`** with `tourism_tax: true` when fiscalizing tourism tax lines.
...
| `tax_type` | Yes | Usually `B` (18% VAT). See [Glossary](/reference/glossary) and [VAT Formulas](/calculations/vat-formulas). |
...
Category B VAT extraction uses taxable amount × 18 / 118, rounded to two decimals.
...
curl -X POST "{{baseUrl}}/sales" \
  ...
  -d '{
    ...
    "purchase_code": "PO0001",
    "customer": {
      "tin_no": "100004527",
      "full_name": "RULIBA CLAYS LTD",
      "telephone": "788312173"
    },
    "dates": { "origin_date": "20260303", "confirmed_at": "20260303145619" },
    "items": [{
      "sequence": 1,
...
```json Payload
{
  ...
  "dates": {
    "origin_date": "20260303",
    "confirmed_at": "20260303145619"
  },
  "items": [
    {
      "sequence": 1,
...
Print **`print.receipt_signature`** and **`print.internal_data`** (and the counters / SDC / MRC as required by your receipt layout). Definitions: [Glossary](/reference/glossary).
...
| `910` | Parameter validation (totals, `orgInvcNo`, tourism rates, types) | Fix payload; for tourism rates use [`PUT /codes/tourism`](/api/codes) or sale-level `tax_rate_f` / `tax_rate_tt` |
