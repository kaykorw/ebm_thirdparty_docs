...
The response is `{ message, data }`. On success, `data` includes `invoice_no`, `original_invoice_no`, `receipt_type`, `status`, `replayed`, and a `print` object with `receipt_no`, `total_receipt_no`, `internal_data`, `receipt_signature`, `receipt_published_at`, `sdc_id`, `mrc_no`, and `tourism`.
...
| `910` | Parameter validation (totals, `orgInvcNo`, tax rates, types) | Fix payload. Live devices require `taxRtF` (18) and `taxRtTt` (0 or 3). See [Tourism Tax and VAT on Fuel](/calculations/tourism-and-fuel). |
