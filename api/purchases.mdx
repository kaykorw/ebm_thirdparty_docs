...
Requires a business API key with scope `purchase`.

When another business sells to you on EBM, you **list** those invoices and **confirm** them. Confirmation is required for input VAT and for automatic stock-in.
...
<ParamField body="supplier_tin" type="string">Required for automatic registration (9 digits)</ParamField>
<ParamField body="supplier_branch_id" type="string">Supplier branch (2 chars)</ParamField>
<ParamField body="supplier_name" type="string">Supplier name</ParamField>
<ParamField body="supplier_invoice_no" type="integer">Supplier invoice number (required for automatic)</ParamField>
<ParamField body="supplier_sdc_id" type="string">Supplier SDC ID</ParamField>
<ParamField body="purchase_date" type="string">`YYYYMMDD`; defaults to today</ParamField>
...
curl -X POST "{{baseUrl}}/purchases/confirm" \
  ...
  -d '{
    ...
    "supplier_tin": "100004527",
    "supplier_name": "RULIBA CLAYS LTD",
    "supplier_invoice_no": 1799,
...
After VSDC `000`, Kayko enqueues stock-in (`purchase`). Purchase refunds enqueue stock-out (`return` / type `12`). Services (`item_type` `3`) are skipped. Seed `POST /stocks/master` first so remaining quantity can update. Do not nest stock in this HTTP request.
