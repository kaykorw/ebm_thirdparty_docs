...
description: Step-by-step Kayko EBM integration checklist: enable EBM, authenticate, initialize the device, then sync codes, items, stock, sales, purchases, and imports.
keywords:
...
flowchart TD
    ...
    B --> C["2. Sync codes and constants"]
...
Pull Kayko’s friendly tables with [`GET /constants/*`](/api/constants) (taxes, payment modes, packaging, quantity units, stock types, refund reasons, and related lists). In parallel, sync the live VSDC code catalog with [`GET /codes`](/api/codes) using `x-last-request-date` (first pull: `20000101000000`). Cache thereafter with `GET /codes/cached`.

If your business is tourism-tax related, or a non-tourism TIN still fails sales with tourism-rate errors, configure [`PUT /codes/tourism`](/api/codes) only after you know which rates the device accepts. Until RRA confirms device-specific values, treat those numbers as **unconfirmed**.
...
| Sync codes/constants before building UI dropdowns | Prevents invalid packaging, tax, and payment codes |
